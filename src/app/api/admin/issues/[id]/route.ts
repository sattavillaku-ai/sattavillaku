...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  // Status change logic
  const { data: currentIssue } = await supabase.from('issues').select('status, published_at').eq('id', params.id).single();
  if (body.status === 'published' && currentIssue?.status !== 'published' && !currentIssue?.published_at) {
    body.published_at = new Date().toISOString();
  }

  const { data, error } = await supabase.from('issues').update(body).eq('id', params.id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
